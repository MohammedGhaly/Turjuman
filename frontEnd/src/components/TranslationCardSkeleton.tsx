      ...
      <div className="flex flex-col h-fit gap-4 px-5 py-[10px] bg-[var(--outer-boxes-bg)] border border-[--box-border] rounded-xl">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-4 items-center">
            <Skeleton key={Math.random()} className="h-8 w-20" />
            <div className="flex gap-2">
              <Skeleton key={Math.random()} className="h-8 w-8 rounded-full" />
              <Skeleton key={Math.random()} className="h-8 w-8 rounded-full" />
            </div>
          </div>
          {/* right */}
        ...
        <div className="flex justify-end gap-4 items-center text-2xl">
          <Skeleton key={Math.random()} className="h-8 w-28" />
        </div>
        <div className="my-2 flex flex-row gap-2">
          <Skeleton key={Math.random()} className="h-8 w-full" />
        </div>
      </div>
    </div>
  );
